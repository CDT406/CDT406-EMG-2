...
# ----------- Sequence Builder -----------
def create_sequences(samples, seq_len=5):
    ...
    for i in range(len(features) - seq_len):
        ...
        y.append(labels[i + seq_len - 1])  # label = last in sequence
...
for person_id, cycles in grouped.items():
    if 4 not in cycles:
        print(f"⚠️ Skipping person {person_id} — no cycle 4 for validation.")
        continue

    val_samples = cycles[4]
    Xv, yv = create_sequences(val_samples, sequence_length)
    if Xv is None:
        print(f"⚠️ Skipping person {person_id} — too few windows in cycle 4.")
        continue
