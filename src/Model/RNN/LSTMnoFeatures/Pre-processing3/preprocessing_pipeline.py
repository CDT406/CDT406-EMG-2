...
def preprocess_signal(window_data):
    """Preprocess EMG signal according to Toro-Ossaba method"""
    signal = window_data['voltage']
    
    # 1. Center the signal (zero-mean)
    centered = signal - np.mean(signal)
    
    # 2. Take absolute value
    rectified = np.abs(centered)
    
    # 3. Apply moving average filter (using Savitzky-Golay filter)
    smoothed = savgol_filter(rectified, window_length=11, polyorder=3)
...
def save_combined_config(output_dir, preprocessing_params, feature_stats, global_window_stats):
    ...
    # Update save location for LSTMnoFeatures
    save_dir = os.path.join('src', 'Model', 'RNN', 'LSTMnoFeatures', 'Saved_models')
    ...
    config = {
        'preprocessing_config': {
            ...
            'savgol_window': 11,  # Added Savitzky-Golay filter parameters
            'savgol_order': 3,
            'normalization': 'min_max'  # Changed to reflect new normalization
        }
    }
...
def main():
    # Update paths for LSTMnoFeatures
    data_dir = 'datasets/official/unprocessed/relabeled_old_dataset'
    output_dir = 'src/Model/RNN/LSTMnoFeatures/Pre-processing3/processed_data'
